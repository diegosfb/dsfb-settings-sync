...
- Create and push a tag like `v1.1.0` to trigger the GitHub Actions release workflow, which builds and attaches a `.vsix` to a GitHub Release.
- Release notes are generated from `CHANGELOG.md`.
- Recommended local flow: update `CHANGELOG.md` → run `release.bat` (bumps version, publishes, commits, tags, pushes).
...
- `v1.1.0` 같은 태그를 생성/푸시하면 GitHub Actions가 자동으로 빌드하고, GitHub Release에 `.vsix`를 첨부합니다.
- 릴리즈 노트는 `CHANGELOG.md`에서 자동 생성됩니다.
- 권장 로컬 플로우: `CHANGELOG.md` 업데이트 → `release.bat` 실행(버전업, 배포, 커밋, 태그, 푸시).
